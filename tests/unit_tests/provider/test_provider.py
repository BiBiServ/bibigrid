...
SERVER_KEYS = {'id', 'name', 'flavor', 'image', 'location', 'volumes', 'has_config_drive', 'progress', 'disk_config',
               'power_state', 'task_state', 'vm_state', 'launched_at', 'terminated_at', 'hypervisor_hostname',
               'instance_name', 'user_data', 'host', 'hostname', 'kernel_id', 'launch_index', 'ramdisk_id',
               'reservation_id', 'root_device_name', 'scheduler_hints', 'security_groups', 'created_at', 'addresses',
               'description', 'key_name', 'metadata', 'networks', 'personality', 'private_v4', 'public_v4', 'public_v6',
               'server_groups', 'status', 'user_id', 'tags', 'interface_ip', 'project_id', 'max_count', 'host_status',
               'min_count'}
FLOATING_IP_KEYS = {'fixed_ip_address', 'floating_ip_address', 'id', 'location', 'floating_network_id', 'port_details',
                    'status', 'created_at', 'updated_at', 'description', 'revision_number',
                    'port_id', 'router_id', 'project_id', 'tenant_id', 'floating_network_id', 'port_details',
                    'dns_domain', 'dns_name', 'tags'}
SUBNET_KEYS = {'id', 'name', 'tenant_id', 'network_id', 'ip_version', 'subnet_pool_id', 'is_dhcp_enabled', 'ipv6_ra_mode',
               'ipv6_address_mode', 'gateway_ip', 'cidr', 'allocation_pools', 'host_routes', 'dns_nameservers',
               'description', 'tags', 'created_at', 'updated_at', 'revision_number', 'project_id'}
NETWORK_KEYS = {'id', 'name', 'project_id', 'is_admin_state_up', 'mtu', 'status', 'subnet_ids', 'is_shared',
                'availability_zone_hints', 'availability_zones', 'ipv4_address_scope_id', 'ipv6_address_scope_id',
                'is_router_external', 'description', 'is_port_security_enabled', 'dns_domain', 'tags', 'created_at',
                'updated_at', 'revision_number', 'project_id'}

FLAVOR_KEYS = {'name', 'description', 'disk', 'is_public', 'ram', 'vcpus', 'swap', 'ephemeral', 'is_disabled',
...
IMAGE_KEYS = {'location', 'created_at', 'updated_at', 'checksum', 'container_format', 'direct_url', 'disk_format',
              ...
              'is_protected', 'locations', 'properties', 'visibility',
              'schema', 'metadata'}
...
class TestProvider(unittest.TestCase):
    ...
    def test_active_server_methods(self):
        for provider, configuration in zip(PROVIDERS, CONFIGURATIONS):
            ...
            with self.subTest(provider.NAME):
                ...
                self.assertTrue(SERVER_KEYS.issubset(set(provider_server.keys())))
                self.assertEqual("bibigrid_test_keypair", provider_server["key_name"])
                self.assertTrue(FLOATING_IP_KEYS.issubset(set(floating_ip.keys())))
    ...
    def test_get_network_get_subnet(self):
        for provider, configuration in zip(PROVIDERS, CONFIGURATIONS):
            with self.subTest(provider.NAME):
                ...
                self.assertTrue(NETWORK_KEYS.issubset(set(network.keys())))
                subnet = provider.get_subnet_by_id_or_name(provider.get_subnet_ids_by_network(network["id"])[0])
                self.assertTrue(SUBNET_KEYS <= set(subnet.keys()))
                network2 = provider.get_network_id_by_subnet(subnet["id"])
    ...
    def test_get_flavor_detail(self):
        for provider, configuration in zip(PROVIDERS, CONFIGURATIONS):
            with self.subTest(provider.NAME):
                self.assertTrue(FLAVOR_KEYS.issubset(set(provider.get_flavor(configuration["flavor"]).keys())))

    def test_get_image(self):
        for provider, configuration in zip(PROVIDERS, CONFIGURATIONS):
            with self.subTest(provider.NAME):
                image = provider.get_image_by_id_or_name(configuration["image"])
                self.assertTrue(IMAGE_KEYS.issubset(set(image.keys())))
    ...
    def test_create_delete_volume(self):
        ...
        """
        for provider in PROVIDERS:
            volume = provider.create_volume(name="test_create_delete_volume", size=1, description="Test run")
            self.assertTrue(volume)
            self.assertTrue(provider.get_volume_by_id_or_name(volume["id"]))
            self.assertTrue(VOLUME_KEYS.issubset(volume.keys()))
            self.assertEqual("test_create_delete_volume", volume["name"])
            self.assertTrue(provider.delete_volume(volume["id"]))
            self.assertFalse(provider.get_volume_by_id_or_name(volume["id"]))
    ...
    if CONFIGURATIONS[0].get("snapshotImage"):
        def test_get_snapshot(self):
            for provider, configuration in zip(PROVIDERS, CONFIGURATIONS):
                with self.subTest(provider.NAME):
                    snapshot = provider.get_volume_snapshot_by_id_or_name(
                        configuration["snapshotImage"])
                    self.assertTrue(SNAPSHOT_KEYS <= set(snapshot.keys()))

        def test_create_volume_from_snapshot_with_delete(self):
            for provider, configuration in zip(PROVIDERS, CONFIGURATIONS):
                with self.subTest(provider.NAME):
                    volume_name = "test_create_volume_from_snapshot_with_delete"
                    volume = provider.create_volume_from_snapshot(snapshot_name_or_id=configuration["snapshotImage"],
